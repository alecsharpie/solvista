#!/usr/bin/env node
/* shot-amphi.mjs — camera-zoom the (1/city) amphitheater and clip it, night + a day
 * control, for the iter-168 concert polish. node shot-amphi.mjs <seed> <warp> <outdir> */
...
const seed = +(process.argv[2]||42), warp = +(process.argv[3]||61), out = process.argv[4]||join(HERE,'../shots/amphi');
...
for(const [name,t] of [['night',0.90],['day',0.35]]){
  ...
  const clip = await p.evaluate(()=>{
    playing=false;
    let a=null; for(let y=0;y<G;y++)for(let x=0;x<G;x++){const c=cells[idx(x,y)];if(c&&c.t===T.CIVIC&&c.kind==='amphitheater'){a=[x,y];break;}}
    if(!a) return null;
    const [wx,wy]=ctr(a[0],a[1]);
    zoom=7; scale=fitScale*zoom; offX=cvs.clientWidth/2-wx*scale; offY=cvs.clientHeight/2-wy*scale;
    render();
    return {x:cvs.clientWidth/2-140,y:cvs.clientHeight/2-140,width:280,height:280};
  });
  if(!clip){console.log('seed',seed,'no amphitheater');continue;}
