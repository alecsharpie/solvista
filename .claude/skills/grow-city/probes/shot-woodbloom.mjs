/* shot-woodbloom — zoomed view of the spring wildflower understory on the forest
 * floor (iter 156). Finds an on-screen forest patch, camera-zooms onto it, and
 * clips spring (flowers) vs summer (control, none). Clock frozen; continuous
 * factor so year drift is harmless, but freeze anyway for a crisp still. */
...
const OUT = resolve(HERE, '.claude/skills/grow-city/shots');
const b = await chromium.launch();
const p = await b.newPage({ viewport: { width: 1440, height: 900 } });
for (const [tag, year] of [['spring', '2035.28'], ['summer', '2035.62']]) {
  ...
  await p.screenshot({ path: join(OUT, `woodbloom-${tag}.png`), clip: { x: 470, y: 230, width: 520, height: 460 } });
}
await b.close();
