#!/usr/bin/env node
/* probe-bridgelamp.mjs — do bridge decks light their rail lamps ONLY at night?
 *
 * PATCH = working tree, BASE = git HEAD. The only change is a draw-only, LITAMT-gated
 * block in the ROAD/bridge case: two warm rail lamps atop the deck + a warm reflection
 * on the water below. Diffing PATCH vs BASE over the bridge cells' screen boxes at a
 * frozen frame isolates the lamps.
 *   TARGET  = c.bridge ROAD cells: change present at NIGHT, ~0 by DAY (LITAMT<0.25, no draw).
 *   CONTROL = non-bridge ROAD cells: ~0 at both (the bridge case is the only edit).
...
const BASE = join(tmpdir(), 'solvista-bridge-base.html');
writeFileSync(BASE, HEADSRC);

const SEEDS = [7, 42, 1234];
const WARP = 61;
const FRAMES = [['night', 0.90], ['day', 0.35]];
const R = 14;                   /* half-box: lamps sit ~9px above centre, reflection ~6px below */
...
async function sample(page, fileUrl, seed, t) {
  ...
  return page.evaluate(({ R, WAVET, seed, warp, t }) => {
    ...
    return { BRIDGE: box(bridge), ROAD: box(road), lit: LITAMT, nb: bridge.length };
...
console.log('\nBRIDGE RAIL LAMPS — PATCH vs HEAD over c.bridge ROAD cells');
console.log('changed-pixel fraction (per-pixel |dRGB| > ' + THR + '), warp 61\n');
console.log('  seed  frame    BRIDGE lamps   ROAD ctl(=0)   nBridge  LITAMT');
...
for (const seed of SEEDS) {
  for (const [fname, t] of FRAMES) {
    ...
    const br = changed(pa.BRIDGE, bs.BRIDGE);
    const roadc = changed(pa.ROAD, bs.ROAD);
    console.log(`  ${seed}  ${fname.padEnd(7)}  ${(br.frac * 100).toFixed(2).padStart(6)}%       ` +
      `${(roadc.frac * 100).toFixed(2).padStart(5)}%        ${String(pa.nb).padStart(3)}     ${pa.lit.toFixed(2)}`);
  }
}
await b.close();
console.log('\nPASS = BRIDGE lamps light at NIGHT, ~0 by DAY (LITAMT<0.25, no draw), ROAD control ~0 both frames.');
